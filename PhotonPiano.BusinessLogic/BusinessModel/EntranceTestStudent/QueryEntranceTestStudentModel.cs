...
public record QueryEntranceTestStudentModel : QueryPagedModel
{
    ...
    public void Deconstruct(out int page, out int pageSize, out string sortColumn, out bool orderByDesc,
        out List<string>? studentsFirebaseIds, out List<decimal>? bandScores, out List<Guid>? entranceTestIds)
    {
        page = Page;
        pageSize = PageSize;
    ...
    public void Deconstruct(out List<string>? studentFirebaseIds, out List<decimal>? bandScores,
        out List<Guid>? entranceTestIds)
    {
